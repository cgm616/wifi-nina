...
/// [`embedded_io`] traits.
pub struct CombinedClient<'a, 'b, T: Transport> {
    wifi: &'a mut Wifi<T>,
    client: &'b mut Client<T>,
}

impl<'a, 'b, T: Transport> CombinedClient<'a, 'b, T> {
...
impl<'a, 'b, T: Transport> Read for CombinedClient<'a, 'b, T> {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        self.client.recv(self.wifi, buf).await
    }
...
impl<'a, 'b, T: Transport> Write for CombinedClient<'a, 'b, T> {
    async fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
        self.client.send(self.wifi, buf).await
    }
}
